...
<div :class="{um__date__class_simple:!isRange, um__date__class_simple_range:isRange, [`um__date__lineH_${plugSize}`]:true, um__date__animationAble:!hoverFirse}" @mouseenter="hoverFirse=false" @click.once="first=false" @click.stop="toFocus">
	...
	<div :id="id" v-if="isRange"
		...
		<div :class="{um__date__input_placeholder1_long_right:iconObj.position!=='left', um__date__input_placeholder1_long_left:iconObj.position==='left'}" v-show="startItem==='' && !importing">开始日期</div>
		...
		<div :class="{um__date__input_placeholder2_long_right:iconObj.position!=='left', um__date__input_placeholder2_long_left:iconObj.position==='left'}" v-show="endItem==='' && !importing">结束日期</div>
	</div>
	...
	<div :class="`um__date__contentBox_${realAlign}`" v-if="!isRange">
		<div :class="`um__date__contentBox_arrow_${realAlign}`"></div>
		<div :class="`um__date__contentBox_c_arrow_${realAlign}`"></div>
		<b-input size="small"></b-input>
	</div>
	<div :class="{[`um__date__contentBox_${realAlign}`]:true, um__date__contentBox_range:true}" v-if="isRange">
		<div :class="`um__date__contentBox_arrow_${realAlign}`"></div>
		<div :class="`um__date__contentBox_c_arrow_${realAlign}`"></div>
...
<script>
export default {
	props:['icon', 'disabled', 'rules', 'validateField', 'size', 'align', 'range', 'time'],
	data(){
		return {
			first:true, // 是否首次加载
	...
	computed:{
		...
		realAlign:function(){
			if(this.align===undefined){
				...
				if(this.align.replace(/\s+/g, '')==='right'){
					return 'right';
				}else{
					return 'left';
				};
	...
	mounted:function(){
		_BD(document, 'click', this.toBlur);
		if(this.verifying){
			this.$watch(function(){
	...
	beforeDestroy(){
		_unBD(document, 'click', this.toBlur);
	}
}
</script>

<style>
.um__date__class_simple {width:160px; min-width:100px; color:#606266; border:1px solid #c0c4cc; border-radius:3px; background:white; display:inline-block; cursor:pointer; position:relative;}
.um__date__class_simple_range {width:280px; min-width:200px; color:#606266; border:1px solid #c0c4cc; border-radius:3px; background:white; display:inline-block; cursor:pointer; position:relative;}
...
.um__date__input_iconRight_default {width:calc(100% - 39px); height:24px; line-height:24px; padding-left:10px; padding-right:29px; outline:none; color:#606266; border:1px solid transparent; border-radius:3px; cursor:pointer; background:transparent; position:absolute; left:-1px; top:-1px;}
.um__date__input_iconRight_big {width:calc(100% - 39px); height:30px; line-height:30px; padding-left:10px; padding-right:29px; outline:none; color:#606266; border:1px solid transparent; border-radius:3px; cursor:pointer; background:transparent; position:absolute; left:-1px; top:-1px;}
.um__date__input_iconRight_small {width:calc(100% - 39px); height:18px; line-height:18px; padding-left:10px; padding-right:29px; outline:none; color:#606266; border:1px solid transparent; border-radius:3px; cursor:pointer; background:transparent; position:absolute; left:-1px; top:-1px;}
.um__date__input_long_iconRight {width:calc(100% - 34px); height:100%; padding-left:5px; padding-right:29px; color:#606266; border:1px solid transparent; border-radius:3px; background:transparent; position:absolute; left:-1px; top:-1px;}
.um__date__input_iconLeft_default {width:calc(100% - 35px); height:24px; line-height:24px; padding-left:30px; padding-right:5px; outline:none; color:#606266; border:1px solid transparent; border-radius:3px; cursor:pointer; background:transparent; position:absolute; left:-1px; top:-1px;}
.um__date__input_iconLeft_big {width:calc(100% - 35px); height:30px; line-height:30px; padding-left:30px; padding-right:5px; outline:none; color:#606266; border:1px solid transparent; border-radius:3px; cursor:pointer; background:transparent; position:absolute; left:-1px; top:-1px;}
.um__date__input_iconLeft_small {width:calc(100% - 35px); height:18px; line-height:18px; padding-left:30px; padding-right:5px; outline:none; color:#606266; border:1px solid transparent; border-radius:3px; cursor:pointer; background:transparent; position:absolute; left:-1px; top:-1px;}
.um__date__rgInput_icon_default {width:calc(50% - 12px); height:20px; line-height:24px; text-align:center; outline:none; color:#606266; border:1px solid transparent; border-radius:3px; cursor:pointer; background:transparent; position:relative; z-index:5;}
...
.um__date__rgInput_icon_small {width:calc(50% - 12px); height:16px; line-height:18px; text-align:center; outline:none; color:#606266; border:1px solid transparent; border-radius:3px; cursor:pointer; background:transparent; position:relative; z-index:5;}
...
.um__date__input_long_iconLeft {width:calc(100% - 34px); height:100%; padding-left:29px; padding-right:5px; color:#606266; border:1px solid transparent; border-radius:3px; background:transparent; position:absolute; left:-1px; top:-1px;}
...
.um__date__class_position_split {position:relative; top:-1px;}
...
.um__date__contentBox_left {width:300px; height:300px; border-radius:3px; border:1px solid #c0c4cc; background:white; position:absolute; left:-1px; top:calc(100% + 10px);}
.um__date__contentBox_right {width:300px; height:300px; border-radius:3px; border:1px solid #c0c4cc; background:white; position:absolute; right:-1px; top:calc(100% + 10px);}
.um__date__contentBox_range {width:400px;}
.um__date__contentBox_arrow_left {width:0; height:0; border:6px solid transparent; border-bottom:10px solid #c0c4cc; position:absolute; left:7px; top:-16px;}
.um__date__contentBox_arrow_right {width:0; height:0; border:6px solid transparent; border-bottom:10px solid #c0c4cc; position:absolute; right:7px; top:-16px;}
.um__date__contentBox_c_arrow_left {width:0; height:0; border:4px solid transparent; border-bottom:10px solid white; position:absolute; left:9px; top:-14px; z-index:5;}
.um__date__contentBox_c_arrow_right {width:0; height:0; border:4px solid transparent; border-bottom:10px solid white; position:absolute; right:9px; top:-14px; z-index:5;}
</style>